...
// Remove the direct import - we'll import it dynamically
...
export default function ResumeBuilder({ initialContent }) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (!element) {
        throw new Error("Resume content not found. Please make sure you have content in your resume.");
      }

      // Check if there's content to generate PDF from
      if (!previewContent || previewContent.trim().length === 0) {
        toast.error("Please add some content to your resume before generating PDF");
        return;
      }
      ...
      const removeProblematicStyles = (el) => {
        ...
        if (el.style) {
          ...
          if (style.includes('oklch')) {
            el.style.cssText = style.replace(/oklch\([^)]+\)/g, '#1a1a1a');
          }
        }
      ...
      // Add the clean element to the DOM temporarily
      ...
      cleanElement.id = 'resume-pdf-clean';
      ...
      const tempElement = document.getElementById('resume-pdf-clean');
      if (tempElement) {
        document.body.removeChild(tempElement);
      }
      
      // Restore original element visibility in case of error
      const element = document.getElementById("resume-pdf");
      if (element) {
